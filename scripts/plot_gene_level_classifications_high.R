...
df <- read.table(
  "../output/ks_results_high/gene_level_classification_with_ks_w_high.tsv",
...
p1 <- ggplot(df, aes(x = ks_used)) +
  geom_histogram(binwidth = 0.2, fill = "#084594", color = "white") +
...
p2 <- ggplot(df %>% filter(!is.na(w_used)), aes(x = w_used)) +
  geom_histogram(binwidth = 0.1, fill = "#7A0177", color = "white") +
...
p6 <- ggplot(df %>% filter(!is.na(w_used)),
  ...
  scale_color_manual(values = c(
    "Positive_selection"  = "#084594",
    "Purifying_selection" = "#ad058b",
