...
All the scripts for pipelien_2 should be placed in `pipeline_2/` folder.
For all intermediate output files, please save them in `output/Ks/` folder.
For any new tools you installed, either add to [./scripts/SETUP.sh](./scripts/SETUP.sh) or write how to isntall it somewhere


The input of this pipeline with be a text file, like one of these:
...
You can use either of this files to run the pipeline scripts in `pipeline_2/` folder.
(They are equivalent, whatever is easier for you to use)

The plan:

- [ ] Write a script that takes one of these file formats as input and generate a list of pairs of genes belonging to the same family (all vs all within each family)
- [ ] write a script tp retrieve CDS sequences from IDs (need to download data files first)
for the genome:
```bash
...
- [ ] write a script to compute Ks using PAML
- [ ] write a script to filter the file to remove pairs with high Ks values (given by user)

For every step done, update the readme we can discuss

*pipeline_1 can be used as reference or example for how the scripts and pipeline should be structured if this helps*