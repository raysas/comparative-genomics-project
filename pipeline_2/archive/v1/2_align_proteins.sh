...
CLUSTALW_CMD=""
        ...
        if command -v mafft &> /dev/null; then
            ALIGN_CMD="mafft --auto"
            echo "   Using alignment tool: MAFFT"
        else
            echo "ERROR: MAFFT not found. Please install:"
            echo "       conda install -c bioconda mafft"
            echo "   or  apt-get install mafft"
            exit 1
        fi

        echo "-- Aligning protein pairs with MAFFT..."
...
for fam_dir in "$INPUT_DIR"/family*; do
    ...
    for pair in "$fam_dir"/*.fa; do
        ...
        if [ -f "$out_file" ]; then
            ...
            # # Run MAFFT alignment
            # if $ALIGN_CMD "$pair" > "$out_file" 2>/dev/null; then
