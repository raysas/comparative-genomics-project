# Input: PANTHER GO enrichment tables
...
# 4. Build 6-panel figure for a given stringency
plot_stringency <- function(stringency) {

  categories <- c("BP", "CC", "MF")
  families   <- c("small", "large")

  input_dir  <- "../../output/family_sizes/GOenrich/"
  output_dir <- "../../figures/family_sizes/"
  ...
  for (cat in categories) {
    for (fam in families) {
      ...
      file_path <- file.path(
        input_dir,
        paste0(cat, "_", fam, "_families_", stringency, "_stringency.txt")
      )
      ...
      fam_label <- case_when(
        fam == "small" ~ "Small family",
        fam == "large" ~ "Large family"
      )
  ...
  ggsave(outfile, final_plot, width = 20, height = 14, dpi = 300)
